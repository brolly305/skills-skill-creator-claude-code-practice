...
Adapted 2026-09-26 for this Linux sandbox from a CLAUDE.md originally written for a
Windows machine (`C:/Users/nateh/...`, a Puppeteer install path, and a `frontend-design`
skill that isn't available here). Changes from the original, noted so they're not
silently lost:

- **No `frontend-design` skill exists in this environment.** The "Always Do First" rule
  below now points at this file's own Anti-Generic Guardrails directly instead of a
  skill invocation. If a `frontend-design` skill becomes available later, prefer it and
  restore that line.
- **Puppeteer isn't installed here; Playwright is** (already proven working elsewhere in
  this repo's session history). `screenshot.mjs` below uses Playwright's Chromium instead,
  loaded via an absolute-path `require` since Node's ESM `import` doesn't honor `NODE_PATH`
  the way CommonJS does — this was verified working end-to-end, not just written and assumed.
- `serve.mjs` and `screenshot.mjs` didn't exist in this repo — both are created fresh
  below, functionally matching what the original file described.
- The Windows Puppeteer/Chrome cache paths are dropped; nothing in this repo needs them.

## Always Do First

- Before writing any frontend code, apply the **Anti-Generic Guardrails** below as a
  checklist, every session, no exceptions.
...
- Screenshot your output, compare against reference, fix mismatches, re-screenshot. Do
  at least 2 comparison rounds. Stop only when no visible differences remain or user
  says so.
...
- Screenshots use **Playwright** (Chromium), not Puppeteer — see the note at the top of
  this file.
...
- Always check the `brand_assets/` folder before designing. It may contain logos, color
...
- This folder does not currently exist in this repo. Create it (`mkdir brand_assets`)
  only when real brand assets actually need to be dropped in — don't create it empty
  just to satisfy this rule.

## Anti-Generic Guardrails

- **Colors:** Never use default Tailwind palette (indigo-500, blue-600, etc.). Pick a
  custom brand color and derive from it.
...
- **Typography:** Never use the same font for headings and body. Pair a display/serif
  with a clean sans. Apply tight tracking (`-0.03em`) on large headings, generous
