...
## Contents
- Instance Right-Sizing (EC2 pricing strategy, Kubernetes right-sizing)
- Serverless Cost Optimization (Lambda memory tuning, provisioned concurrency, API Gateway, Graviton)
- Data Transfer Cost Traps (NAT gateway, cross-region, CloudFront vs S3)
- Storage Optimization (S3 lifecycle, storage tier comparison)
- Container Optimization (multi-stage Docker builds)
- CI/CD Cost Reduction (build caching, self-hosted runners, parallel jobs)
- Database Pricing Comparisons (PostgreSQL, Redis)
- Real-World Pricing Scenario (before/after e-commerce example)
